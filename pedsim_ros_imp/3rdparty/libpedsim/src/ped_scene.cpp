...
void Ped::Tscene::moveAgentsWithSocial(double h) {
  // get current time with ros time
  ros::Time currentTime = ros::Time::now();
  // first update states
  for (Tagent* agent : agents) agent->updateState();
  ROS_INFO("1. used time: %f", (ros::Time::now() - currentTime).toSec());
  // then update forces
  for (Tagent* agent : agents) agent->computeForces();
  ROS_INFO("2. used time: %f", (ros::Time::now() - currentTime).toSec());
  ...
  ROS_INFO("3. used time: %f", (ros::Time::now() - currentTime).toSec());
}
